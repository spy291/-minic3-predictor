...
class AdvancedClinicalPredictor:
    ...
    def _bootstrap_auc(self, y_true, y_prob, n_bootstrap=1000):
        """Bootstrap法计算AUC置信区间 - 稳定版"""
        aucs = []
        n = len(y_true)
        
        # 转换为numpy数组
        y_true_np = np.array(y_true) if not isinstance(y_true, np.ndarray) else y_true
        y_prob_np = np.array(y_prob) if not isinstance(y_prob, np.ndarray) else y_prob
        
        for _ in range(n_bootstrap):
            idx = np.random.choice(n, n, replace=True)
            if len(np.unique(y_true_np[idx])) > 1:
                try:
                    aucs.append(roc_auc_score(y_true_np[idx], y_prob_np[idx]))
                except:
                    continue
    ...
    def train(self, df):
        ...
        with st.spinner('正在训练高级预测模型 (集成学习 + 交叉验证)...'):
            ...
            self.cm_data = {
                'response': confusion_matrix(y_response_test, y_response_pred),
...
with st.sidebar:
    ...
    st.markdown("---")
    st.caption(f"© 2024 MINIC3预测系统 v4.5")
...
if page == "🏥 临床数据总览":
    ...
    with tab2:
        ...
        with col2:
            fig = px.box(df, x='ECOG评分', y='PFS_月', color='ECOG评分', title='ECOG评分与PFS关系')
            st.plotly_chart(fig, use_container_width=True)
            
            fig = px.violin(df, x='PD-L1表达', y='PFS_月', color='PD-L1表达', title='PD-L1表达与PFS关系', box=True)
            st.plotly_chart(fig, use_container_width=True)
    ...
    with st.form("prediction_form"):
        ...
        if submitted:
            input_data = pd.DataFrame([{
                '年龄': age, '性别': gender, 'BMI': 24,
                'ECOG评分': ecog, '剂量水平(mg/kg)': dose,
                '既往治疗线数': 1, '转移部位数': metastasis,
                '肝转移': 1 if liver_mets else 0, '脑转移': 0, '骨转移': 0,
                'PD-L1表达': pdl1, 'TMB(mut/Mb)': tmb,
                'NLR': nlr, 'LDH(U/L)': ldh, 'CRP(mg/L)': crp,
                '白蛋白(g/L)': 38, 'PLR': 150, 'PNI': 45,
                '肿瘤类型': '非小细胞肺癌', 'TNM分期': 'IV期',
                'MSI状态': 'MSS', 'LIPI评分': '中风险'
            }])
            
            # 编码
            input_data['性别编码'] = 0 if gender == '男' else 1
            input_data['PD-L1编码'] = {'阴性(<1%)':0, '低表达(1-49%)':1, '高表达(≥50%)':2}[pdl1]
            input_data['肿瘤类型编码'] = 0
            input_data['TNM分期编码'] = 1
            input_data['MSI状态编码'] = 0
            input_data['LIPI评分编码'] = 1
            input_data['年龄xECOG'] = age * ecog
            input_data['剂量xPDL1'] = dose * input_data['PD-L1编码']
            input_data['NLRxLDH'] = nlr * (ldh / 100)
...
# ==================== 页面3：模型性能分析 ====================
    ...
    with tab1:
        ...
        with col1:
            ...
            st.info(f"95% CI: [{st.session_state.model.roc_data['response']['auc_ci'][0]:.3f}, {st.session_state.model.roc_data['response']['auc_ci'][1]:.3f}]")
        ...
        with col2:
            ...
            st.info(f"95% CI: [{st.session_state.model.roc_data['ae']['auc_ci'][0]:.3f}, {st.session_state.model.roc_data['ae']['auc_ci'][1]:.3f}]")
    ...
    with tab3:
        ...
        with col2:
            st.subheader("AE预测混淆矩阵")
            cm_ae = st.session_state.model.cm_data['ae']
            fig = px.imshow(cm_ae, text_auto=True,
                           x=['预测无AE', '预测有AE'],
                           y=['实际无AE', '实际有AE'],
    ...
    with tab4:
        ...
        with col2:
            ...
            fig.update_layout(title='不良事件预测校准曲线')
            st.plotly_chart(fig, use_container_width=True)
    ...
    with tab5:
        ...
        with col1:
            ...
            metrics_df = pd.DataFrame([
                ...
                ['AUC', f"{st.session_state.metrics['response']['auc']:.4f}"],
        ...
        with col2:
            ...
            metrics_df = pd.DataFrame([
                ...
                ['AUC', f"{st.session_state.metrics['ae']['auc']:.4f}"],
...
# ==================== 页面4：生存分析 ====================
    ...
    for group in groups:
        ...
        for t in time_points:
            ...
            if at_risk > 0:
                prob = (at_risk - len(group_data[group_data['PFS_月'] == t])) / at_risk
                survival_prob.append(prob)
            else:
                survival_prob.append(0)
        
        cum_survival = np.cumprod(survival_prob)
        ax.step(time_points, cum_survival, where='post', label=str(group), linewidth=2)
    ...
    for i, group in enumerate(groups):
        with cols[i]:
            median = df[df[group_by] == group]['PFS_月'].median()
            st.metric(str(group), f"{median:.1f} 月")
    ...
    if 'last_prediction' in st.session_state:
        ...
        with col1:
            ...
            for key, value in patient_info.items():
                if not any(x in key for x in ['编码', '_']):
...
<div style="text-align: center; color: #718096; padding: 1rem;">
    <p>© 2024 MINIC3智能预测系统 · 专业版 v4.5</p>
    <p style="font-size: 0.8rem;">基于集成机器学习的临床决策支持平台</p>
</div>
""", unsafe_allow_html=True)
